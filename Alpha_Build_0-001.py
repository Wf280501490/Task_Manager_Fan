...
def open_main_app():
    global logged_in, task_table, user_data, user_exp_label, task_manager
    if logged_in:
        task_manager = tk.Toplevel(root)
        task_manager.title("Task Manager")

        view_history_button = tk.Button(task_manager, text="View Task History", font=("Helvetica", 14), command=view_task_history)
        view_history_button.pack()

        settings_button = tk.Button(task_manager, text="Settings", font=("Helvetica", 14), command=create_settings_panel)
        settings_button.pack()

        # Create a button to create a task
        create_popup_button = tk.Button(task_manager, text="Create Task", font=("Helvetica", 14), command=create_task_popup)
        ...
        task_table = ttk.Treeview(task_manager, columns=("Name", "Description", "Difficulty", "Status", "Date"), show="headings")
        ...
        user_data = {"Total EXP": 0}
        user_exp_label = tk.Label(task_manager, text=f"Total EXP: {user_data['Total EXP']}", font=("Helvetica", 14))
        user_exp_label.pack()

        # Add a "Complete" button
        complete_button = tk.Button(task_manager, text="Complete Selected Task(s)", font=("Helvetica", 14), command=mark_as_completed)
...
def create_account():
    ...
    if username and password:
        ...
        if existing_username:
            status_label.config(text='Username already exists. Please choose a different username.')
        else:
...
def mark_as_completed():
    global user_data, user_exp_label
...
def update_history_text():
    if history_text:
        ...
        history_window.deiconify()  # Show the existing history window
...
def save_settings(settings_popup):
...
def main():
    global root, username_entry, password_entry, status_label
    ...
    login_button = tk.Button(root, text="Login", command=check_login_status)
    login_button.pack()
